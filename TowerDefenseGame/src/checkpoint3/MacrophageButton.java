...
/**
 * class for adding button to spawn astronauts
 *
...
public class MacrophageButton extends GameObject implements Clickable
{
    //Fields
    ...
    public MacrophageButton(Control control, GameState state)
    {
        super();
     ...
     * @return true if mouse clicks button
     */
    public boolean consumeClick()
    {
        //test to see if click is over tower button
        Point mouseLoc = state.getMouseLocation();
        if (mouseLoc.x >= 715 && mouseLoc.x <= 790 && mouseLoc.y >= 400 && mouseLoc.y <= 475
                && state.getMoney() >= 15 && state.gettowerCount() <= 20)
        {
            //If clicked add new satellite object
            state.addGameObject(new Macrophage(control, state));
            state.inctowerCount();
            return true;
        }
        else if(mouseLoc.x >= 715 && mouseLoc.x <= 790 && mouseLoc.y >= 400 && mouseLoc.y <= 475
                && state.getMoney() <= 15)
        {
    ...
    public void draw(Graphics g)
    {
        ...
        drawCenteredImage(g, control.getImage("WBC.png"), 752, 437, 0.69);
        if(clicked)
        {
            g.setColor(new Color(1.0f, 0, 0,errorTint));
            g.setFont(new Font("Arial", Font.BOLD, 8));
            g.drawString("NOT ENOUGH MONEYS", 700, 495);
